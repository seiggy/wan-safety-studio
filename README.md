...
A portable, private Azure ML WAN/ComfyUI baseline for an **existing customer landing zone**, provisioned with Terraform. The local portal binds only to **http://127.0.0.1:51881/**. Do not run the upstream public deployment.
...
Terraform creates an owned resource group, private storage, an RBAC Key Vault, Premium ACR, App Insights, two managed identities, scoped role assignments, private endpoints and a GPU NSG association. **It does not create or own your VNet, subnets, private DNS zones, DNS links/resolver, VPN or Log Analytics workspace.** Existing DNS zones receive private endpoint zone groups; existing subnet associations are changed only under the dedicated-subnet contract.
...
# Portal: http://127.0.0.1:51881/
.\scripts\Invoke-WanSafetyStudio.ps1 -Action Stop
.\scripts\Invoke-WanSafetyStudio.ps1                 # default: live Status
```

**Deploy and Prepare leave compute/NAT/PIP absent.** Deploy requires explicit persistent-cost approval. Prepare downloads/verifies code and model assets, builds and tests the image on local CPU, pushes through Entra authentication and registers immutable private AML inputs. It does not allocate a GPU.
...
**The Terraform port is locally validated only, not live-deployed.** The predecessor application completed a private Spot A100 WAN job on 2026-09-17 with `PT30M`, one instance, 512x512 WebM (17 frames at 16 fps) and a CPU-converted H.264 MP4 (1.063 seconds), with full decoding and final compute/NAT/PIP release verified. This is application provenance, not evidence that the new customer landing-zone Terraform port has been Azure-tested. Live provisioning, private routing, DNS/RBAC propagation, image/model access, quota/capacity and generated-video quality require a later explicitly approved customer test.
