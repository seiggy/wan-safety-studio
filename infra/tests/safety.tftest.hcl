...
run "default_off_private_foundation" {
  ...
  assert {
    condition     = alltrue([for rule in azurerm_network_security_group.compute.security_rule : rule.direction != "Inbound" || rule.access != "Allow" || rule.source_address_prefix == var.gpu_subnet_cidr]) && length([for rule in azurerm_network_security_group.compute.security_rule : rule if rule.direction == "Inbound" && rule.access == "Deny" && rule.priority == 4096 && rule.destination_port_range == "*"]) == 1
    error_message = "No public/VPN application or SSH ingress: only node-internal inbound followed by deny all."
  }
  assert {
    condition     = alltrue([for rule in azurerm_network_security_group.compute.security_rule : (try(length(rule.destination_port_range), 0) > 0) != (try(length(rule.destination_port_ranges), 0) > 0)]) && length([for rule in azurerm_network_security_group.compute.security_rule : rule if rule.destination_address_prefix == "AzureMachineLearning" && rule.protocol == "Udp" && rule.destination_port_range == "5831"]) == 1
